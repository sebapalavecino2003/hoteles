...
export default function EmployeeLayout() {
  ...
  if (!isAuthenticated || user?.role !== 'employee') return <Navigate to="/" replace />
    ...
    <div style={{ display: 'flex', minHeight: 'calc(100vh - 64px)' }}>
      <aside style={{ width: 220, background: '#1a1a2e', padding: '16px 0' }}>
        <div style={{ padding: '0 20px', marginBottom: 20, color: '#e94560', fontWeight: 700, fontSize: 14 }}>
          Employee Panel
          <div style={{ color: '#888', fontSize: 11, fontWeight: 400, marginTop: 4 }}>
            Hotel #{user.hotel_id}
          </div>
        </div>
        <nav>
          {[
            ...
            { to: '/employee/room-types', label: 'Room Types' },
            { to: '/employee/rooms', label: 'Rooms' },
            { to: '/employee/reservations', label: 'Reservations' },
          ].map(({ to, label }) => (
            <NavLink
              key={to}
              to={to}
